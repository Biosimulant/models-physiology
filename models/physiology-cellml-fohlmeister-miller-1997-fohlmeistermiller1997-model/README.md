# Fohlmeister, Miller, 1997

**Source**: [physiome](https://models.physiomeproject.org/workspace/fohlmeister_miller_1997)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - path: ../../models/physiology-cellml-fohlmeister-miller-1997-fohlmeistermiller1997-model
    alias: model
```

## Tags

physiology, cellml, physiome, auto-generated
