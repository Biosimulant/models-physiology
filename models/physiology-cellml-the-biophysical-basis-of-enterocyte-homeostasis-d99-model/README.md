# The biophysical basis of enterocyte homeostasis

**Source**: [physiome](https://models.physiomeproject.org/workspace/d99)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - repo: Biosimulant/models
    alias: model
    manifest_path: models/physiology-cellml-the-biophysical-basis-of-enterocyte-homeostasis-d99-model/model.yaml
```

## Tags

physiology, cellml, physiome, auto-generated
