# Mohr, Taylor, Newell, 2008

**Source**: [physiome](https://models.physiomeproject.org/workspace/mohr_taylor_newell_2008)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - path: ../../models/physiology-cellml-mohr-taylor-newell-2008-mohrtaylornewell2008-model
    alias: model
```

## Tags

physiology, cellml, physiome, auto-generated
