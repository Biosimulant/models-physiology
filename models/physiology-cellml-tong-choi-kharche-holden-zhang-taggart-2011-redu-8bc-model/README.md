# Tong, Choi, Kharche, Holden, Zhang, Taggart, 2011 - Reduced Model

**Source**: [physiome](https://models.physiomeproject.org/workspace/8bc)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - repo: Biosimulant/models
    alias: model
    manifest_path: models/physiology-cellml-tong-choi-kharche-holden-zhang-taggart-2011-redu-8bc-model/model.yaml
```

## Tags

physiology, cellml, physiome, auto-generated
