# Degradation of Bugbuster RNA 2

**Source**: [physiome](https://models.physiomeproject.org/workspace/SVP_00000002)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - repo: Biosimulant/models
    alias: model
    manifest_path: models/physiology-cellml-degradation-of-bugbuster-rna-2-svp00000002-model/model.yaml
```

## Tags

physiology, cellml, physiome, auto-generated
