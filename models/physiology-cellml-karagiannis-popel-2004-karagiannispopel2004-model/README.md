# Karagiannis, Popel, 2004

**Source**: [physiome](https://models.physiomeproject.org/workspace/karagiannis_popel_2004)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - path: ../../models/physiology-cellml-karagiannis-popel-2004-karagiannispopel2004-model
    alias: model
```

## Tags

physiology, cellml, physiome, auto-generated
