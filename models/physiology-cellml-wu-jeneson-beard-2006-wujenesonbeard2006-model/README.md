# Wu, Jeneson, Beard, 2006

**Source**: [physiome](https://models.physiomeproject.org/workspace/wu_jeneson_beard_2006)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - path: ../../models/physiology-cellml-wu-jeneson-beard-2006-wujenesonbeard2006-model
    alias: model
```

## Tags

physiology, cellml, physiome, auto-generated
