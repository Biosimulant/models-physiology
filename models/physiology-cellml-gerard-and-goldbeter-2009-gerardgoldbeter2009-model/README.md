# Gerard and Goldbeter 2009

**Source**: [physiome](https://models.physiomeproject.org/workspace/gerard_goldbeter_2009)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - path: ../../models/physiology-cellml-gerard-and-goldbeter-2009-gerardgoldbeter2009-model
    alias: model
```

## Tags

physiology, cellml, physiome, auto-generated
