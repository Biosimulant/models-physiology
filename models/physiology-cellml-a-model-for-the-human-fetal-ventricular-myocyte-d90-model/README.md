# A model for the human fetal ventricular myocyte electrophysiology

**Source**: [physiome](https://models.physiomeproject.org/workspace/d90)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - repo: Biosimulant/models
    alias: model
    manifest_path: models/physiology-cellml-a-model-for-the-human-fetal-ventricular-myocyte-d90-model/model.yaml
```

## Tags

physiology, cellml, physiome, auto-generated
