# Guyton, Autonomics, 2008

**Source**: [physiome](https://models.physiomeproject.org/workspace/guyton_autonomics_2008)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - path: ../../models/physiology-cellml-guyton-autonomics-2008-guytonautonomics2008-model
    alias: model
```

## Tags

physiology, cellml, physiome, auto-generated
