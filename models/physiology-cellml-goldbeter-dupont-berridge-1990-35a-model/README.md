# Goldbeter Dupont Berridge 1990

**Source**: [physiome](https://models.physiomeproject.org/workspace/35a)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - repo: Biosimulant/models
    alias: model
    manifest_path: models/physiology-cellml-goldbeter-dupont-berridge-1990-35a-model/model.yaml
```

## Tags

physiology, cellml, physiome, auto-generated
