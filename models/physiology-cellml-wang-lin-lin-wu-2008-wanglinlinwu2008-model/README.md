# Wang, Lin, Lin, Wu, 2008

**Source**: [physiome](https://models.physiomeproject.org/workspace/wang_lin_lin_wu_2008)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - path: ../../models/physiology-cellml-wang-lin-lin-wu-2008-wanglinlinwu2008-model
    alias: model
```

## Tags

physiology, cellml, physiome, auto-generated
