# Fox, Mcharg, Gilmour, 2002

**Source**: [physiome](https://models.physiomeproject.org/workspace/fox_mcharg_gilmour_2002)
**Standard**: cellml
**Authors**: physiome

## Description

Faithful wrapper of upstream CELLML model.


## Usage

This model was auto-generated from the physiome repository.

```yaml
# In a space.yaml wiring file:
models:
  - path: ../../models/physiology-cellml-fox-mcharg-gilmour-2002-foxmcharggilmour2002-model
    alias: model
```

## Tags

physiology, cellml, physiome, auto-generated
